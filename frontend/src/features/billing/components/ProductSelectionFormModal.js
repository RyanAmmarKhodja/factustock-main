...
import { getProducts } from "../../stock/api/productApi";
import { PageHeader, Card, Alert } from "../../../shared/components/UI";
...
export default function ProductSelectionFormModal({ onSave, onClose }) {
  const [filters, setFilters] = useState({ search: undefined, active: true });
  ...
  useEffect(() => {
    fetchProducts(1, { search: undefined, active: true });
  }, [fetchProducts]);
