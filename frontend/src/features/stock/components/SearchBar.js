...
export default function SearchBar({ onSearch }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      // Build params without "undefined" keys/values — Axios serializes
      // `key: undefined` as the literal string "undefined" in the query string.
      const params = {};
      if (search.trim()) params.search = search.trim();
      // Unchecked: only active products. Checked: don't send "active" at all
      // → backend applies no filter and returns active + archived.
      if (!includeArchived) params.active = true;
      onSearch(params);
    }, 350);
