...
/// <summary>
/// QuestPDF template for Bon de Livraison / Réception / Commande.
/// Layout: company header → client block → title → line table → totals → signature zone.
/// </summary>
public class BonTemplate(BonDocument data) : IDocument
{
    // ── Colours (same palette as InvoiceTemplate) ─────────────────────────────
    ...
    public DocumentMetadata GetMetadata()
    {
        ...
        meta.Title  = $"{data.BonTypeLabel} {data.BonNumber}";
    ...
    void ComposeHeader(IContainer container)
    {
        ...
        {
            // Top line: address (left) + date (right)
            col.Item().Row(row =>
            {
                ...
                row.RelativeItem().AlignRight().Column(title =>
                {
                    title.Item()
                        ...
                        .Padding(12)
                        .AlignCenter()
                        .Text(data.BonTypeLabel)
                        .FontSize(14).Bold().FontColor(White);

                    title.Item().PaddingTop(6).AlignRight()
                        .Text($"N° {data.BonNumber}")
                        .FontSize(11).Bold().FontColor(TextDark);

                    title.Item().AlignRight()
                        .Text($"Date : {data.BonDate:dd/MM/yyyy}")
                        .FontSize(9).FontColor(TextMuted);
                });
            });
    ...
    void ComposeTotals(IContainer container)
    {
        container.Column(col =>
        {
            ...
            col.Item()
                ...
                .Row(r =>
                {
                    r.RelativeItem().Text("TOTAL TTC").FontColor(White).Bold().FontSize(10);
                    r.RelativeItem().AlignRight().Text(FormatDA(data.TTC)).FontColor(White).Bold().FontSize(10);
                });
        });
    }
    ...
    void ComposeSignatures(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Border(1).BorderColor(BorderGray).Padding(12).Column(sig =>
            {
                sig.Item().Text("Cachet et signature de l'émetteur")
                    .FontSize(8).Bold().FontColor(TextMuted);
                sig.Item().PaddingTop(40).Text("").FontSize(8); // space for stamp
            });
            ...
            row.RelativeItem().Border(1).BorderColor(BorderGray).Padding(12).Column(sig =>
            {
                sig.Item().Text("Cachet et signature du destinataire")
                    .FontSize(8).Bold().FontColor(TextMuted);
                sig.Item().PaddingTop(40).Text("").FontSize(8);
            });
        });
    }
